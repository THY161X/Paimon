...
pub fn has_capability() -> bool {
    ...
    unsafe { libusb::libusb_has_capability(constants::LIBUSB_CAP_HAS_CAPABILITY) != 0 }
}

/// Tests whether the running `libusb` library supports hotplug.
pub fn has_hotplug() -> bool {
    GlobalContext::default().as_raw();
    unsafe { libusb::libusb_has_capability(constants::LIBUSB_CAP_HAS_HOTPLUG) != 0 }
}

/// Tests whether the running `libusb` library has HID access.
pub fn has_hid_access() -> bool {
    GlobalContext::default().as_raw();
    unsafe { libusb::libusb_has_capability(constants::LIBUSB_CAP_HAS_HID_ACCESS) != 0 }
}
...
pub fn supports_detach_kernel_driver() -> bool {
    GlobalContext::default().as_raw();
    unsafe {
        libusb::libusb_has_capability(constants::LIBUSB_CAP_SUPPORTS_DETACH_KERNEL_DRIVER) != 0
    }
}
...
pub fn set_log_level(level: LogLevel) {
    unsafe {
        libusb::libusb_set_debug(GlobalContext::default().as_raw(), level.as_c_int());
    }
}
